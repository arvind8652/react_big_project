import { apiRequestUrls } from '../config/apiConfig';
import { Api } from '../services/apiService';

export const getControlStructure = () => {
	return Api.get(apiRequestUrls.getControlStructureSecondaryMarket);
};

export const getDependentDataAccount = () => {
	const postObject = {
		data: {
			fieldListID: 20063,
			dependentValue: {
				client_id: 'ANDREWC'
			}
		}
	};
	return Api.post(apiRequestUrls.getDependentDataSecondaryMarket, postObject);
};

export const getDependentDataBank = () => {
	const postObject = {
		data: {
			FieldListID: 20064,
			dependentValue: {
				scheme: 'ANDREWC'
			}
		}
	};
	return Api.post(apiRequestUrls.getDependentDataSecondaryMarket, postObject);
};

export const getDependentDataSecurity = () => {
	const postObject = {
		data: {
			fieldListID: 20062,
			dependentValue: {
				asset_type: '',
				issuer: ''
			}
		}
	};
	return Api.post(apiRequestUrls.getDependentDataSecondaryMarket, postObject);
};

export const getXIRROrYieldCalculation = () => {
	const postObject = {
		data: {
			faceValue: 100,
			interest: 7.5,
			matDate: '30-12-2021',
			price: 26,
			price100: 26,
			quoteBasis: 'P',
			security: 'BONDMY',
			settleDate: '01-01-2020',
			xirrType: 'Y',
			yield: null,
			priceOrYield: 'P'
		}
	};
	return Api.post(apiRequestUrls.getXIRROrYieldCalculation, postObject);
};

export const postSecondaryMarketOrder = () => {
	const postObject = {
		data: {
			dealRequisition: {
				dealId: '',
				scheme: 'ANDREW',
				tranType: 'BUY',
				assetType: 'BND',
				security: 'BONDMYNEW',
				rate: 38,
				purYield: null,
				valueDate: '2019-12-29',
				fcyGrossVal: null,
				grossVal: null,
				fcyGrossInt: null,
				grossInt: null,
				broker: 'AFFINBROKER',
				brokeragePer: null,
				fcyBrkCommn: null,
				nettVal: null,
				fcyNettVal: null,
				orderInstruction: 'GTC',
				OtherInstruction: 'AORN',
				bankAccForINM: '110100-PNB1-PHP',
				instrType: 'C',
				account: '110100-PNB1-PHP',
				bank: 'CIMB',
				chqNumber: '1233667',
				chqDate: '2020-01-03',
				branch: 'ANDHERIW',
				withholdingTaxPer: null,
				remarks: '',
				currency: 'NGN',
				accCurrency: 'PHP'
			},
			documentInfo: {
				lstDocumentInfo: [
					{
						documentType: 'A',
						docNameDataValue: 'FDFD',
						noDocRequired: 2,
						expiryYN: 'N',
						docStatus: 'U',
						submissionDate: '2021-03-22',
						expiryDate: null,
						actionDate: null,
						fileName: 'LightBill1.png',
						fileType: 'png',
						fileDescription: 'LightBill1.png',
						progName: 'ORDERSMADD',
						documentsetupid: 'DOCFORORDER',
						fileString:
							'',
						srlNo: 1,
						isdeferred: 'N'
					},
					{
						documentType: 'A',
						docNameDataValue: 'ABCD',
						noDocRequired: 2,
						expiryYN: 'Y',
						docStatus: 'U',
						submissionDate: '2021-03-08',
						expiryDate: '2025-03-04',
						actionDate: null,
						fileName: 'AAdharCard.png',
						fileType: 'png',
						fileDescription: 'AAdharCard.png',
						progName: 'ORDERSMADD',
						documentsetupid: 'DOCFORORDER',
						fileString:
							'',
						srlNo: 2,
						isdeferred: 'N'
					},
					{
						documentType: 'A',
						docNameDataValue: 'NMVB',
						noDocRequired: 2,
						expiryYN: 'Y',
						docStatus: 'D',
						submissionDate: null,
						expiryDate: null,
						actionDate: '2019-12-29',
						fileName: '',
						fileType: '',
						fileDescription: '',
						progName: 'ORDERSMADD',
						fileString: '',
						srlNo: 3,
						isdeferred: 'Y',
						documentsetupid: 'DOCFORORDER'
					}
				]
			},
			attachment: [
				{
					refType: 'ORDERSMADD',
					refId: null,
					fileDescription: 'Pan Card',
					fileName: 'PanCard.png',
					fileSize: '5.4KB',
					mimetype: 'image/png',
					fileString:
						'',
					attachmentFor: null,
					sessionId: ''
				}
			]
		},
    };
    return Api.post(apiRequestUrls.postSecondaryMarketOrder, postObject);
};

const getSecurityListDetails = () => {
    const postObject = {
        data: {
            security:["ZOMATOSEC"],
            scheme:"ANDREW"
			}
    };
    return Api.post(apiRequestUrls.getSecurityListDetailsSecondaryMarket, postObject);
};
