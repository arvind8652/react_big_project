...
import { faBirthdayCake, faEnvelopeSquare, faPhoneAlt, faComment, faFile } from "@fortawesome/pro-light-svg-icons";

export const CalederBadge = ({ calendarList = [], selectedDate }) => {
    ...
    return (
      <React.Fragment>
        <h4 className="item-heading">
          {today === selectedDate._d.toDateString()
            ? "Today"
            : tomorrow === selectedDate._d.toDateString()
            ? "Tomorrow"
            : null}{" "}
          {selectedDate._d.toDateString()}
        </h4>
        {Object.keys(list).map((key) => {
          let sortedList = [];
          if (list[key] && list[key].length) {
            sortedList = list[key].sort((a, b) => new Date(b["nextRunDate"]) - new Date(a["nextRunDate"]));
            return (
              <React.Fragment>
