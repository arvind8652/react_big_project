...
export default function DocumentsDetail({
	...
	const updateRequestObject = (uploadedDocInfo, latestRowData = []) => {
		// console.log('uploadedDocInfo--------', uploadedDocInfo);
		// console.log('latestRowData--------', latestRowData);
		let withDocIdRecord = [];
		// if (uploadedDocInfo) {
		if (Array.isArray(uploadedDocInfo) && uploadedDocInfo?.length > 0) {
	...
	const generateRowData = (data, data1) => {
		...
		// console.log('test data1---1111-------', data1);
		let count = 0;
	...
	useEffect(() => {
		let onEditUploadedDocInfoResp;
		if (
			documentRequestObject?.data?.uploadedDocInfo &&
			'uploadedDocInfo' in documentRequestObject?.data
		) {
			onEditUploadedDocInfoResp = documentRequestObject?.data?.uploadedDocInfo;
			delete documentRequestObject?.data?.uploadedDocInfo;
		}
		getDocumentDetails(documentRequestObject).then((res) => {
			setRowData(
				// generateRowData(res.data?.documentDetailsModel, res.data?.uploadedDocInfo?.lstDocumentInfo)
				generateRowData(
					res.data?.documentDetailsModel,
					action !== 'copy' && action !== 'profileCopy'
						? onEditUploadedDocInfoResp
							? onEditUploadedDocInfoResp
							: res.data?.uploadedDocInfo?.lstDocumentInfo
						: []
				)
			);
		});
		...
		<Card
			...
			extra={
				<Typography.Title
					level={5}
					onClick={() => {
						rowData?.length > 0 && setModal1Visible(true);
					}}
				>
					{showUploadButton && (
						<Typography.Link disabled={rowData?.length <= 0}>
