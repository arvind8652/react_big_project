...
export default function AddressDetails({
	...
	console.log('data getting-----------', formData);
	useEffect(() => {
	...
	useEffect(() => {
		...
		// }
		setDisableMailingAddress(formData?.disablePrermanentAddressField);
	...
	// -----------------------------------------------
	const populateNewState = async () => {
		getState(formData?.permCountry)
			.then((resp) => setStates(resp.data))
			// .then(() => populateNewCities())
			.catch((err) => console.log('populateState', err));
	};
	const populateNewCities = async () => {
		getCity(formData?.permanentState)
			.then((resp) => setCities(resp.data))
			.catch((err) => console.log('populateCity', err));
	};
	useEffect(() => {
		if (states && formData?.permCity && (action === 'edit' || action === 'convert')) {
			populateNewCities();
		}
	}, [formData?.permCity, states]);
	// -----------------------------------------------
	...
	// -----------------------------------------------
	const populateNewMailState = async () => {
		getState(formData?.mailCountry)
			.then((resp) => setMailStates(resp.data))
			// .then(() => populateNewCities())
			.catch((err) => console.log('populateMailState', err));
	};
	const populateNewMailCities = async () => {
		getCity(formData?.mailState)
			.then((resp) => setMailCities(resp.data))
			.catch((err) => console.log('populateMailCity', err));
	};
	useEffect(() => {
		if (mailStates && formData?.mailCity && (action === 'edit' || action === 'convert')) {
			populateNewMailCities();
		}
	}, [formData?.mailCity, mailStates]);
	// -----------------------------------------------

	useEffect(() => {
		if (action === 'edit' || action === 'convert') {
			// populateMailingStateAndCity();
			populateNewMailState();
		}
	...
	// -----------------------------------------------
	const populateNewOfficeState = async () => {
		getState(formData?.compCountry)
			.then((resp) => setCompStates(resp.data))
			// .then(() => populateNewCities())
			.catch((err) => console.log('populateOfficeState', err));
	};
	const populateNewOfficeCities = async () => {
		getCity(formData?.compAdd3)
			.then((resp) => setCompCities(resp.data))
			.catch((err) => console.log('populateOfficeCity', err));
	};
	useEffect(() => {
		if (compState && formData?.compCity && (action === 'edit' || action === 'convert')) {
			populateNewOfficeCities();
		}
	}, [formData?.compCity, compState]);
	// -----------------------------------------------
	useEffect(() => {
		if (action === 'edit' || action === 'convert') {
			// populateOfficeStateAndCity();
			populateNewOfficeState();
		}
