...
const PersonalDetails = ({
	...
	const [familiesNew, setFamiliesNew] = useState([]);
	...
	const createNewFamily = async () => {
		try {
			...
			setFamiliesNew([{ family: response?.data?.transactionId, name: familyName }, ...familiesNew]);
			form.setFieldsValue({ surName: familyName });

			onValuesChange({ surName: response?.data?.transactionId });
			// onFamilyNameSearch(familyName);
		} catch (error) {
		} finally {
	...
	useEffect(() => {
		if (csObject?.FamilyName) {
			...
			setFamiliesNew(csObject?.FamilyName.lookupValue.lookUpValues);
			// console.log('sssss-----', csObject?.FamilyName.lookupValue.lookUpValues?.length);
			maxFamilyNameCount =
				csObject?.FamilyName.lookupValue.lookUpValues?.length < 200
					? csObject?.FamilyName.lookupValue.lookUpValues?.length
					: 200;
		}
	}, [csObject?.FamilyName]);
	...
	const onFamilyNameSearch = (val) => {
		let regExp = new RegExp(val, 'i');
		// console.log('value-------', val);
		let familyNameMatched = [];

		// console.log('before----------', familyNameMatched);
		families.map((_) => {
			if (val && _.name.match(regExp)) {
				familyNameMatched.push(_);
			}
		});

		// console.log('eee-----------', familyNameMatched);
		if (familyNameMatched.length == 0) {
			familyNameMatched = familiesNew;
		}
		// console.log('ddd-----------', familyNameMatched);
		let familyNameMatchedCount = familyNameMatched.length;
		if (familyNameMatchedCount < maxFamilyNameCount) {
			families.slice(0, maxFamilyNameCount - familyNameMatchedCount).map((_) => {
				// if (_.name != val) {
				if (!_.name.match(regExp)) {
					familyNameMatched.push(_);
				}
			});
		}
		setFamiliesNew(familyNameMatched);
		val ? setFamilyName(val) : setFamilyName(null);
		// console.log(familyNameMatched);
	};
					...
					<Col span={7}>
						...
						>
							<Select
								...
								// onSearch={(val) => (val ? setFamilyName(val) : setFamilyName(null))}
								onSearch={(val) => onFamilyNameSearch(val)}
								// onChange={(val) => onFamilyNameSearch(val)}
								notFoundContent={<></>}
									...
									return (
										...
										>
											...
											<div className='dropdown-custom-options'>
												{familiesNew?.slice(0, maxFamilyNameCount)?.map((item, index) => (
