...
export default function BankDetailsFormCard({
	...
	const mulitpleAccNumChk = (acc) => {
		const newAccNum =
			bankDetailsList?.length > 0 &&
			bankDetailsList.filter((num) => {
				return acc === num.accountNo;
			});
		setCount(newAccNum?.length);
	};

	const checkAccNum = (rule, val, callback) => {
		if (val) {
			if (count >= 1) {
				return Promise.reject(new Error('Account number already present'));
			} else {
										...
										<Col span={8}>
													...
													rules
														? [
																...
																...[
																	...
																	{
																		validator: formData?.type === 'CS' ? checkAccNum : ''
																	}
																]
														  ]
														: []
											...
											>
												<Input
													...
													onChange={(evt) => {
														onChange(evt.target.value, 'accountNo', index);
														mulitpleAccNumChk(evt.target.value);
													}}
													// maxLength='16'
