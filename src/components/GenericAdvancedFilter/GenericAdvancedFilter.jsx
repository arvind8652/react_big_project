import { Select, Input, Checkbox, Space, Slider } from 'antd';
...
const AdvFilterBody = ({ setAdvFilters, advFilters, advFilterData }) => {
				...
				return (
					...
					>
						<Space style={{ paddingLeft: '12px' }} direction='Horizontal'>
							{field.dataValues?.map((eachOption) => (
								<Checkbox value={eachOption?.key}>{eachOption?.value}</Checkbox>
							))}
						</Space>
					</Checkbox.Group>
				);
...
const GenericAdvancedFilterDrawer = ({
...
export default GenericAdvancedFilterDrawer;
