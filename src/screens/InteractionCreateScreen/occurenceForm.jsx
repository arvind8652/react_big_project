...
const OccurenceForm = (props) => {
	const children = [];
	for (let i = 10; i < 36; i++) {
		children.push(<Option key={i.toString(36) + i}>{i.toString(36) + i}</Option>);
	}
	// {
	//   props.csObject.invitee &&
	//     props.csObject.invitee.map((item, index) => (
			...
			>
					...
					{(fields, { add, remove }) => (
						<>
							<GenericCard
								cardTitle='Occurence'
								menuFlag={1}
								controlList={(cardButtonObject, (cardButtonObject.onAdd = add()))}
							>
								{/* <Card
                className="interaction-occ-card-type"
                title={<h2>Occurence</h2>}
                extra={
                  <Button
                    type="text"
                    onClick={() => {
              ...
              > */}
                        ...
                        >
                          ...
                          >
                            {props.csObject.InteractionType &&
                              ...
                              props.csObject.InteractionType.dropDownValue
                                .length > 0 &&
                              props.csObject.InteractionType.dropDownValue.map(
                                (item, index) => (
                        ...
                        <Form.Item
                          name="closeReason"
                          label={
                            <div className="interaction-text">Close Reason</div>
                          }
                        >
                          <Select
                            className="interaction-filter-dropdown"
                          ...
                          >
												...
												<Row gutter={16} className='w-90' justify='space-between'>
													<Col className='gutter-row' span={16}>
														<DatePicker
															...
															size='large'
															// defaultValue={moment(
															//   props.csObject.StartDate &&
															//     props.csObject.StartDate.defaultvalue,
															//   "DD-MM-YYYY"
															// )}
															format='DD-MM-YYYY'
															disabledDate={(d) =>
                            ...
                            props.csObject.Reminder.dropDownValue.map(
                              (item, index) => (
                                <Option value={item.dataValue} key={index}>
                                  {item.displayValue}
                                </Option>
