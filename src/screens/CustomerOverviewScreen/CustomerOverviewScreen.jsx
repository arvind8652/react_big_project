...
import { Card, Col, Popover, Row, Select, Tabs } from 'antd';
import { Area, TinyArea, Scatter, Column, Pie } from '@ant-design/charts';
import {
	getAgingAnalysisApi,
	getConStrikeRateApi,
...
import { faAngleDoubleDown, faAngleDoubleUp, faStar } from '@fortawesome/pro-regular-svg-icons';
import axios from 'axios';
import moment from 'moment';
import { checkValidGraphData, exportJSON, kFormatter } from '../../utils/utils';
import {
	ScButtonText,
	ScModal,
	ScRate,
		...
		<div className='oppor-amount-stats-container'>
			<div className='stats'>
				...
				<div className='target-amount'>
					{/* {amount && currencyFormatter(amount, "en-US")} */}
					{'$'} {amount && <RupeeOrNonRupee amount={amount} />}
				</div>
			</div>
					...
					<Metrics justify='center'>
						{data &&
							(mode === 'count' ? (
								...
								<>
									{'$'} <RupeeOrNonRupee amount={data.targetAmount} />
								</>
							))}
					</Metrics>
	...
	return (
		...
		>
			...
			<ScButtonText
				...
				color='#354081'
				onClick={() => {
					authorizeModule(authorizeCode, CONSTANTS.authorizeCode.download) && downloadRecords();
				}}
			>
				...
				&nbsp;&nbsp;
				{authorizeModule(authorizeCode, CONSTANTS.authorizeCode.download) ? 'Download report' : ''}
			</ScButtonText>
		</ScModal>
	);
};
const CustomerOverviewScreen = ({ leftPanel }) => {
	...
	const RenderRecentTrendCard = () => {
		...
		const RenderTooltip = (data) => {
			...
			const ValueAttr = () => (
				<TooltipAttribute align='top' justify='space-between'>
					...
					<Col span={14}>
						<strong>
							{'$'} {data && <RupeeOrNonRupee amount={data.value} />}
						</strong>
					</Col>
				</TooltipAttribute>
			);
		...
		const RenderAssetManagement = () => (
			<div className='conversion-won'>
				<div className='won-amount'>
					{'$'}
					<RupeeOrNonRupee amount={conStrikeRateData.wonAmount} />
				</div>
				<div className='title'>Asset Under Management</div>
				<div className='graph-section'>
					{conRateGraphConfig && <TinyArea {...conRateGraphConfig} />}
				</div>
				<div className='subtitle'>You manage $75,000 average per customer</div>
			</div>
		);
		const RenderMovedUp = () => {
		...
		const RenderRevenue = () => (
			<div className='conversion-won'>
				<div className='won-amount'>
					{/* {currencyFormatter(conStrikeRateData.wonAmount, "en-US")} */}
					{'$'} <RupeeOrNonRupee amount={conStrikeRateData.wonAmount} />
				</div>
				...
				<div className='subtitle'>You earn $12,000 average per customer</div>
			</div>
		);
		return (
			...
			const ValueAttr = () => {
				...
				return (
					<>
						...
						{plottingField === 'value' && (
							// <Row>({currencyFormatter(data.targetAmount, "US")})</Row>
							<Row>
								({'$'} <RupeeOrNonRupee amount={data.targetAmount} /> )
							</Row>
						)}
					...
					{data.map((item) => (
						<TooltipAttribute align='top' justify='space-between'>
							...
							<Col span={13}>
								...
								<strong>
									{'$'} {item && <RupeeOrNonRupee amount={item.value} />}
								</strong>
							</Col>
						</TooltipAttribute>
					))}
			...
			const RenderTooltip = (data) => {
				...
				const ValueAttr = () => (
                  ...
                  "US"
								...
								<RupeeOrNonRupee
									amount={plottingCategory === 'expected' ? data.targetAmount : data.investamount}
								/>
							</strong>
						</Col>
					</TooltipAttribute>
...
export default connect(mapStateToProps)(CustomerOverviewScreen);
